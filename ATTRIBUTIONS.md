...
| [Leonxlnx/unlazy](https://github.com/Leonxlnx/unlazy) | MIT | fable-workflows: four ideas adapted as prose into verification-before-completion (acceptance checks written before the work, positive control for absence checks, abandoned is not done, final reconciliation against the original request). Skipped: the gate-check/lint/dispatch Node tooling, approval store, Depth Tree orchestration and Stop hook — overlap with subagent-driven-development and parallel-plans |
