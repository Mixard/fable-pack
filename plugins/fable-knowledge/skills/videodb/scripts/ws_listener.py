...
Arguments:
  output_dir  Directory for output files
              (default: $VIDEODB_EVENTS_DIR, else $XDG_STATE_HOME/videodb,
              else ~/.local/state/videodb)

Options:
  --clear     Clear the events file before starting (use for a new session)

Output files:
  <output_dir>/videodb_events.jsonl  - All WebSocket events (JSONL)
  <output_dir>/videodb_ws_id         - WebSocket connection ID
  <output_dir>/videodb_ws_pid        - Process ID for termination

Output (first line, for parsing):
  WS_ID=<connection_id>

Examples:
  python scripts/ws_listener.py &                      # background, append mode
  python scripts/ws_listener.py --clear &              # fresh session
  python scripts/ws_listener.py --clear /tmp/mydir &   # custom dir
  kill "$(cat ~/.local/state/videodb/videodb_ws_pid)"  # stop
"""
import os
import sys
import json
...
# Track whether this is the first connection (for clearing events)
